...
public class PartialMatchIdentifyTest {
    ...
    public void testStringPartialMatch() {
        Taker<String> abc = Taker.commit(Lexical.string("abc"));
        ...
        assertEquals(ResultType.PARTIAL, result.type(), "Taker.commit(Lexical.string) should return PARTIAL on partial match");
    }
    ...
    public void testSequencePartialMatch() {
        ...
        Taker<Character> abc = Taker.commit(a.skipThen(b).skipThen(c));
        Input input = Input.of("abd");
    ...
    public void testRepeatPartialMatch() {
        Taker<Character> a = Lexical.chr('a');
        Taker<List<Character>> a3 = Taker.commit(a.repeat(3));
